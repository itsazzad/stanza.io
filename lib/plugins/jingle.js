...
module.exports = function (client) {
    ...
    jingle.on('send', function (data) {
        if (data.signal) {
            client.sendConnectSignal(data, function (err, result) {
                if (err) {
                    console.error('connect:signal:outgoing:error', err);
                    client.emit('connect:signal:outgoing:error', err);
                }
            });
        } else {
    ...
    client.on('connect:signal:incoming:chat', function (data) {
        jingle.process(data);
    });
    client.on('connect:signal:incoming:error', function (data) {
        jingle.process(data);
    });
